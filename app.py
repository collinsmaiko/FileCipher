...
def get_tiktok_opts(download_id):
    ...
    return {
        ...
        # Best separate video+audio (mp4+m4a) then fallback to best mp4, then anything.
...
def import_video():
    if request.method == "POST":
        ...
        try:
            ...
            return render_template("import_video.html", error=f"Failed to download: {str(e)}")
